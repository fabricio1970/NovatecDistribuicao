...
set "SERVER_ROOT=%ROOT%..\NovoServidor"
set "VALIDATOR=%SERVER_ROOT%\validar_conexoes_design_time.bat"
set "OUTPUT=%ROOT%servidor"
set "LOGDIR=%ROOT%logs"
set "LOG=%LOGDIR%\CompilarServidor.log"
...
if not exist "!VALIDATOR!" (
    echo ERRO: validador design-time nao encontrado: !VALIDATOR!
    exit /b 10
)
...
echo Validando conexoes design-time do Servidor...
call "%VALIDATOR%" > "%LOG%" 2>&1
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" (
    echo ERRO: validacao design-time falhou. Exit code: %RC%
    echo Log: %LOG%
    exit /b %RC%
)

echo Compilando Servidor Release/Win32...
echo DPROJ: %DPROJ%
echo EXE: %OUTPUT%\NovatecServidor.exe
...
if not "%RC%"=="0" (
    echo ERRO: compilacao do Servidor falhou. Exit code: %RC%
    echo Log: %LOG%
    exit /b %RC%
)

if not exist "%OUTPUT%\NovatecServidor.exe" (
    echo ERRO: EXE do Servidor nao foi gerado.
    exit /b 40
)

del /q "%OUTPUT%\NovatecServidor.map" "%OUTPUT%\NovatecServidor.drc" 2>nul
